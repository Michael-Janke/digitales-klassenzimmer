    ...
    render() {
        ...
        if(medium.qr) {
            return <div className={classNames(classes.root,classes.qr)} ref={this.myRef}>
                <a className={classes.link} href={medium.src} target="_blank" rel="noopener noreferrer">{medium.src}</a>
                <QRCode 
                    renderAs="svg"
                    value={medium.src}
                    size="100%"
            ...
            return (
                <iframe className={classes.root} ref={this.myRef} src={medium.src} title="iframe" />
            );
        }
    }
}
