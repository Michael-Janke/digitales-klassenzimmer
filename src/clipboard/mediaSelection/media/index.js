...
class MenuDecorator extends React.Component {
    ...
    render() {
        const { medium, url, isDragging, connectDragSource, style, allowDelete } = this.props;
        const { src } = medium;
...
MenuDecorator = DragSource("MEDIA", cardSource, (connect, monitor) => ({
    connectDragSource: connect.dragSource(),
    isDragging: monitor.isDragging()
}))(MenuDecorator);

MenuDecorator = connect(({me}, {medium}) => ({
    allowDelete: me.role === 'teacher' || medium.senderId === me.id
}), mapDispatchToProps)(MenuDecorator);

export default MenuDecorator;